...
# ╔═╡ eb497a68-a987-11ee-3367-b1b7eb1f7a73
begin
	using DifferentialEquations
	using Plots
end

# ╔═╡ 13f90bc7-bd5a-4d93-b4f2-a98c683f532f
function expsel!(du,u,p,t)
	du[1] = u[2] * u[1] * (1 - u[1])
	du[2] = -p[1] * u[1] * u[2]
end

# ╔═╡ 52e1531e-2634-4a65-a325-e03eb6bed62e
begin
	s0 = 0.01
	x0 = 1e-3
	α = s0
...
# ╔═╡ dbe3a3f8-0ad8-4e44-bc81-e5a4ea4795a5
sol, tspan = let
	u0 = [x0, s0]
	tspan = (0, 20/s0)
	p = [α]
	prob = ODEProblem(expsel!,u0,tspan,p)
	solve(prob), tspan
end;

# ╔═╡ 437a5554-e0fd-4860-a0a0-f2cd27c5ab12
xapprox(t, β, x0, s) = x0*β / (x0 + (β-x0)*exp(-s*t))

# ╔═╡ 817e0a63-ff57-4632-be4b-c7b58f45c08b
# conv_time(s0, ν, β, x0) = -1/s0 * log(x0/ν/β)
conv_time(s0, ν, β, x0) = 1/s0 * log(β/x0 * ν/(1-ν))

# ╔═╡ 477dfcfa-76bc-4192-a4ca-596eba9ab26d
conv_time(s0, .5, β, x0)

# ╔═╡ 0e8dfd75-4254-4094-9e02-381e3b5abc2e
ν = .8

# ╔═╡ d65e8892-c24a-40f9-a54d-7293add18991
let p = plot()
	tvals = range(tspan..., length=100) |> collect
	# plot(tvals, sol(tvals))
	xvals = [sol(t)[1] for t in tvals]

	plot(tvals, xvals)
	plot!(tvals, map(t -> xapprox(t, β, x0, s0), tvals))

	hline!([ν*β], label="", line=(:black, :dash))
	vline!([conv_time(s0, ν, β, x0)], label="", line=(:black))
end
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "9c04cc3c05025e6cc203f39f479f242c16436233"
