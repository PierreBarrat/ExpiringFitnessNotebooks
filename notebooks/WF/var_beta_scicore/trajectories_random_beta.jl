...
begin
	mβvals = 0.2:0.2:0.8 |> collect
	β2vals = map(mβvals) do m
		r = range(m^2*1.0001, m*0.99, length=2)
		L = div(length(r), 2)
		r[1:L]
	end

	βvals =[(mβ=m, β2=v) for (j, m) in enumerate(mβvals) for v in β2vals[j]]
	md"**Distributions of β**"
end

# ╔═╡ aa039c92-7a8b-4acc-8100-0f808d8fc6d1
ρvals = @chain range(log(1e-3), log(1e-1), length=4) exp.(_) |> collect

# ╔═╡ 1110d7fa-9c96-4b40-ae60-2396a08d4db0
parameters = map(Iterators.product(βvals, ρvals)) do (βv, ρ)
	(mβ = βv.mβ, β2 = βv.β2, ρ=ρ)
end |> x -> vcat(x...)
...
diversity = let
	...
	for (i, p) in enumerate(parameters)
        ...
        Ne = get_Ne(p.ρ, p.β2)
        T = max(round(Int, 1000*Ne), 5_000)

		@time diversity = simulate(p.mβ, p.β2, p.ρ; Δt, T)
		# diversity
		df = DataFrame()
		for x in diversity
			push!(df, x)
