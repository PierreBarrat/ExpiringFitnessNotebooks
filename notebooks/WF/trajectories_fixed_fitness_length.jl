...
function length_histogram(trajectories, params)
	...
	vline!([1/params.ρ], label="1/ρ", line=(2, :black))
	vline!([1/params.s], label="1/s", line=(2, :black, :dashdot))

	plot!(title = "s = $(params.s), ρ = $(round(params.ρ, sigdigits=2)), s/ρ = $(round(params.s/params.ρ; sigdigits=2))", xlabel = "time")
...
plt_dict = map(eachrow(dat)) do r
	(r.ρ, r.s) => length_histogram(r.trajectories, r)
end |> Dict
