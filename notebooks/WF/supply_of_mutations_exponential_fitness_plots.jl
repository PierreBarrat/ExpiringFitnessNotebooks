...
datdir = "data_supply_of_mutations_exponential_fitness/"
...
# ╔═╡ 5c6d0fd4-4d34-45e1-93e5-5377232fa08e
β2(s, α) = s / (s + α) * 2*s / (2*s + α)
...
function do_plot(ρ, β, L, α, data)
	s = compute_s(β, α)
	v1 = round(L*β2(s, α), sigdigits=2)
	v2 = round(s/ρ, sigdigits=2)
	Ne = round(1/ρ/β2(s, α), sigdigits=2)
	
	p = plot()

	plot!(data.t, data.varpos_5, label = "thr: 0.05")
	plot!(data.t, data.varpos_strict, label = "thr: 1/N")
	hline!([1/v1], line=(:black, :dashdot, 3, 0.5), label = "1/β^2/L")
	vline!([Ne], line = (:black, :dash), label = "Ne")
	
	plot!(
		xlabel = "time",
		ylabel = "Nb var. positions (scaled)",
		title = "ρ=$ρ, β=$β, Lβ^2=$(v1), s/ρ=$(v2), Ne=$Ne",
		frame=:box,
		ylim = (-0.02, 1.02),
	)
